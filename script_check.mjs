...
const shopItems=[{name:'Sharper Starter Bow',desc:'+1 permanent starting damage.',base:22,key:'dmg',apply:()=>meta.dmg++},{name:'Thicker Cloak',desc:'+8 permanent starting HP.',base:24,key:'hp',apply:()=>meta.hp++},{name:'Blue Flask',desc:'+8 permanent starting mana and tiny regen.',base:22,key:'mana',apply:()=>meta.mana++},{name:'Starter Skill Token',desc:'Begin each run with one random offensive skill.',base:1000,costs:[1000,2000,3000],key:'startSkill',max:3,apply:()=>meta.startSkill++}];
...
function makeSaveSnapshot(){
  const keepState=['time','wave','waveTime','lastWave','spawnTimer','kills','bossSpawnedWave','bossCount','mapIndex','portalSpawned','finalBossSpawned','bossesDefeatedThisMap','totalXpGained','heartSpawnTimer','lastShot','currentProfile','waveWaitingForBoss','waveSpawnRemaining','waveAdvanceLock'];
  const keepPlayer=['class','x','z','hp','maxHp','mana','maxMana','speed','level','xp','nextXp','money','damage','fireRate','attackSpeedLevel','bullets','pierce','bulletSpeed','magnet','manaRegen','crit','spread','speedLevel','guardRange','guardRangeLevel','upgrades','spells','cooldowns'];
  const snap={version:2,savedAt:Date.now(),state:{},player:{},meta:clonePlain(meta),entities:snapshotEntities()};
  keepState.forEach(k=>snap.state[k]=state[k]);
  keepPlayer.forEach(k=>snap.player[k]=JSON.parse(JSON.stringify(player[k])));
  return snap;
}
...
function restoreGameSnapshot(snap){
...
function gameOver(){sfx('death');clearControls();state.running=false;if(!state.paidRun){meta.gold+=player.money;state.paidRun=true}ui.finalStats.innerHTML=`Level ${player.level}, wave ${state.wave}, ${state.kills} monsters defeated, $${player.money} collected.<br>Merchant coins: $${meta.gold}`;renderShop();ui.gameOverModal.classList.remove('hidden')}
