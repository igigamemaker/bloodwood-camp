...
const state={running:false,paused:false,time:0,wave:1,waveTime:0,lastWave:1,spawnTimer:0,enemies:[],bullets:[],drops:[],particles:[],spells:[],enemyShots:[],orbiters:[],portal:null,lastShot:0,kills:0,paidRun:false,bossSpawnedWave:0,bossCount:0,mapIndex:Math.max(0,Math.min(mapDefs.length-1,Number(startMapIndex)||0)),portalSpawned:false,finalBossSpawned:false,bossesDefeatedThisMap:0,totalXpGained:0,heartSpawnTimer:15,currentProfile:null,waveWaitingForBoss:false,waveSpawnRemaining:0,waveAdvanceLock:0};
const meta={gold:0,dmg:0,wheelSpins:0,wheelSpells:[]};
function normalizeMerchantMeta(){
  meta.gold=Math.max(0,Math.floor(Number(meta.gold)||0));
  meta.dmg=Math.max(0,Math.min(10,Math.floor(Number(meta.dmg)||0)));
  meta.wheelSpins=Math.max(0,Math.min(3,Math.floor(Number(meta.wheelSpins)||0)));
  meta.wheelSpells=Array.isArray(meta.wheelSpells)?meta.wheelSpells.slice(0,3):[];
  delete meta.hp; delete meta.mana; delete meta.startSkill;
}
function resetMerchantMeta(){Object.keys(meta).forEach(k=>delete meta[k]);Object.assign(meta,{gold:0,dmg:0,wheelSpins:0,wheelSpells:[]});}
const HERO_BASE_STATS={mage:{hp:100,dmg:12.5},guard:{hp:125,dmg:15},archer:{hp:85,dmg:10}};
const MERCHANT_META_KEY='bloodwood_merchant_meta_v2';
function loadMerchantMeta(){try{const saved=JSON.parse(localStorage.getItem(MERCHANT_META_KEY)||'{}');if(saved&&typeof saved==='object')Object.assign(meta,saved);normalizeMerchantMeta();}catch(e){console.warn('Merchant meta load failed',e);resetMerchantMeta()}}
function saveMerchantMeta(){try{normalizeMerchantMeta();localStorage.setItem(MERCHANT_META_KEY,JSON.stringify(meta));}catch(e){console.warn('Merchant meta save failed',e)}}
function saveMerchantMetaToCurrentProfile(){
  try{
    if(!state.currentProfile)return false;
    const saves=readProfileSaves();
    const existing=saves[state.currentProfile]||makeSaveSnapshot();
    existing.meta=clonePlain(meta);
    existing.savedAt=Date.now();
    saves[state.currentProfile]=existing;
    return writeProfileSaves(saves);
  }catch(e){console.warn('Merchant profile sync failed',e);return false;}
}
function persistMerchantAndProfile(){
  saveMerchantMeta();
  saveMerchantMetaToCurrentProfile();
}
loadMerchantMeta();
...
const merchantWheelPool=['fireball','frost','lightning','meteor','dagger','wisps','spores','mines','force'];
const merchantSpellNames={fireball:'Fireball',frost:'Frost Nova',lightning:'Storm Acorn',meteor:'Meteor Rain',dagger:'Dancing Dagger',wisps:'Spirit Wisps',spores:'Poison Spores',mines:'Rune Mines',force:'Force Field'};
function grantMerchantSpell(id,announce=false){
  if(!id)return;
  if(id==='meteor')player.spells.meteor=1;
  else if(id==='dagger'){player.spells.dagger=Math.min(5,(player.spells.dagger||0)+1);syncDaggers();}
  else if(id==='lightning'||id==='force')player.spells[id]=Math.min(5,(player.spells[id]||0)+1);
  else player.spells[id]=(player.spells[id]||0)+1;
  if(announce)toast('Wheel granted '+(merchantSpellNames[id]||id));
}
function applyMerchantRunBonuses(){
  normalizeMerchantMeta();
  player.damage+=(meta.dmg||0);
  for(const id of meta.wheelSpells||[])grantMerchantSpell(id,false);
}
const shopItems=[{name:'Damage Training',desc:'+1 starting damage. Max level 10.',key:'dmg',max:10,costFor:(level)=>(level+1)*100,apply:()=>{meta.dmg=Math.min(10,(meta.dmg||0)+1);if(state.running)player.damage+=1;}}];
const player={class:'mage',x:0,z:0,r:.35,hp:110,maxHp:110,mana:100,maxMana:100,speed:6,level:1,xp:0,nextXp:24,money:0,damage:11,fireRate:.48,attackSpeedLevel:0,bullets:1,pierce:0,bulletSpeed:13,magnet:2.2,manaRegen:11,crit:.05,spread:.13,invuln:0,speedLevel:0,guardRange:1.45,guardRangeLevel:0,upgrades:{},spells:{fireball:0,frost:0,lightning:0,dagger:0,wisps:0,spores:0,mines:0,force:0,meteor:0},cooldowns:{fireball:0,frost:0,lightning:0,meteor:0,wisp:0,spore:0,mine:0,force:0}};player.mesh=sprite(textures.hero,1.85);
...
function addDungeonDecor(){
  const m=currentMap();
  if(m.decor==='camp'){
    // User-marked yellow torch positions only; previous generated torches are skipped on map 1.
    [[-30.5,-18.3],[-4.5,-17.5],[26.3,-17.0],[39.2,-7.7],[-29.4,1.1],[-24.7,8.5],[-2.8,19.3],[33.1,14.0],[26.3,21.2]].forEach(([x,z])=>addTorch(x,z));
  }else{
    [[-27,-16],[-7,-8],[16,-16],[-18,7],[8,10],[36,3],[47,10],[-28,-1]].forEach(([x,z],i)=>addWallTorch(x,z));
  }
  if(m.decor==='camp'){
    // Map 1: remove extra generated props (obstacle blocks, bookshelves, blood/puddles/webs/chains).
    // Leave only skulls/skeletons/bones as requested.
    [[-24,-3],[-13,13],[6,3],[20,-9],[31,11],[40,14],[-22,-2],[-8,12],[18,-8],[33,0]].forEach(([x,z])=>addSkeleton(x,z));
    [[-27,-7],[13,13],[35,13]].forEach(([x,z])=>addBone(x,z));
  }else{
...
const bossProfiles=[{tex:'bossImported1',name:'Ironclad Behemoth',color:0x38bdf8,size:2.7,baseHp:1000,baseDmg:38,spd:1.45},{tex:'bossImported2',name:'Bone Harvester',color:0x84cc16,size:2.9,baseHp:1250,baseDmg:42,spd:1.15},{tex:'bossImported3',name:'Winged Death',color:0x93c5fd,size:2.85,baseHp:1500,baseDmg:46,spd:1.35},{tex:'bossImported4',name:'Forest Guardian',color:0xff5a2f,size:3.0,baseHp:1750,baseDmg:50,spd:1.2},{tex:'bossImported5',name:'Void Wraith',color:0xbef264,size:2.95,baseHp:2500,baseDmg:54,spd:1.1},{tex:'bossImported6',name:'Goblin Warlord',color:0x86efac,size:3.0,baseHp:2000,baseDmg:58,spd:1.05},{tex:'bossImported7',name:'Swamp Behemoth',color:0xf97316,size:3.1,baseHp:2250,baseDmg:62,spd:0.95},{tex:'bossImported8',name:'Plague Bringer',color:0xa855f7,size:2.85,baseHp:2500,baseDmg:66,spd:1.22},{tex:'bossImported9',name:'Toxic Horror',color:0xf472b6,size:2.7,baseHp:2750,baseDmg:70,spd:1.25},{tex:'bossImported10',name:'Moss Colossus',color:0x22d3ee,size:3.0,baseHp:3500,baseDmg:74,spd:0.9},{tex:'bossImported11',name:'Frost Titan',color:0xa78bfa,size:2.85,baseHp:3000,baseDmg:78,spd:1.25},{tex:'bossImported12',name:'Crystal Golem',color:0x22c55e,size:2.95,baseHp:3250,baseDmg:82,spd:1.05},{tex:'bossImported13',name:'Deep Sea Tyrant',color:0xe5e7eb,size:2.9,baseHp:3500,baseDmg:86,spd:1.0},{tex:'bossImported14',name:'Stonecrusher Ogre',color:0xf97316,size:2.85,baseHp:3750,baseDmg:90,spd:1.05},{tex:'bossImported15',name:'Dread Reaper',color:0xef4444,size:3.0,baseHp:4500,baseDmg:94,spd:1.15},{tex:'bossImported16',name:'Arachnid Queen',color:0xf97316,size:3.25,baseHp:4000,baseDmg:98,spd:0.95},{tex:'bossImported17',name:'Fallen Vampire',color:0x86efac,size:3.05,baseHp:4250,baseDmg:102,spd:1.1},{tex:'bossImported18',name:'Lich Lord',color:0x22c55e,size:3.0,baseHp:4500,baseDmg:106,spd:1.05},{tex:'bossImported19',name:'Night Gargoyle',color:0xa855f7,size:3.4,baseHp:4750,baseDmg:110,spd:1.0},{tex:'bossImported20',name:'Shadow Doombringer',color:0xcbd5e1,size:3.35,baseHp:5500,baseDmg:114,spd:0.85},{tex:'bossImported21',name:'Magma Elemental',color:0x22d3ee,size:3.2,baseHp:5000,baseDmg:118,spd:0.95},{tex:'bossImported22',name:'Lava Behemoth',color:0xa3e635,size:3.0,baseHp:5250,baseDmg:122,spd:1.2},{tex:'bossImported23',name:'Fire Titan',color:0x38bdf8,size:3.25,baseHp:5500,baseDmg:126,spd:0.9},{tex:'bossImported24',name:'Pumpkin Fiend',color:0x84cc16,size:3.4,baseHp:5750,baseDmg:130,spd:0.82},{tex:'bossImported25',name:'Infernal Overlord',color:0xd946ef,size:4.15,baseHp:10000,baseDmg:134,spd:1.08}];
...
const finalBossProfile={tex:'bossImported25',name:'Infernal Overlord',color:0xd946ef,size:4.35,baseHp:10000,baseDmg:205,spd:1.08,fireCd:2.25,fireSpeed:5.1,fireDmg:104};
const abilities=[{name:'Twin Arrows',icon:abilityIconData.twin,rarity:'rare',desc:'Max Lv 5. Lv1 shoots 2 arrows, then +1 arrow per level up to 6. Unavailable for Guard.',weight:9,apply:()=>{if(player.class!=='guard'&&(player.upgrades.twinArrows||0)<5){player.upgrades.twinArrows=(player.upgrades.twinArrows||0)+1;player.bullets=Math.min(6,1+player.upgrades.twinArrows)}}},{name:'Faster Draw',icon:abilityIconData.faster,rarity:'basic',desc:'+10% attack speed. Max level 5.',weight:9,apply:()=>{if(player.attackSpeedLevel<5){player.attackSpeedLevel++;player.fireRate*=.90}}},{name:'Sharper Runes',icon:abilityIconData.sharper,rarity:'basic',desc:'Projectile damage. Max Lv 5: +10%, +15%, +20%, +25%, +30% total.',weight:9,apply:()=>{const lvl=player.upgrades.sharperRunes||0;if(lvl<5){const bonuses=[1.10,1.15,1.20,1.25,1.30];const prev=lvl>0?bonuses[lvl-1]:1;const next=bonuses[lvl];player.damage=player.damage/prev*next;player.upgrades.sharperRunes=lvl+1}}},{name:'Fleet Boots',icon:abilityIconData.boots,rarity:'basic',desc:'+10% move speed. Max level 5.',weight:7,apply:()=>{if(player.speedLevel<5){player.speedLevel++;player.speed*=1.10}}},{name:'Moon Magnet',icon:abilityIconData.magnet,rarity:'basic',desc:'Collect drops from farther away.',weight:7,apply:()=>{player.magnet+=1.25;player.upgrades.moonMagnet=(player.upgrades.moonMagnet||0)+1}},{name:'Warm Soup',icon:abilityIconData.heart,rarity:'basic',desc:'+30 max HP and heal 30.',weight:7,apply:()=>{player.maxHp+=30;player.hp=Math.min(player.maxHp,player.hp+30);player.upgrades.warmSoup=(player.upgrades.warmSoup||0)+1}},{name:'Piercing Candle',icon:abilityIconData.candle,rarity:'rare',desc:'Archer only. Max Lv 5: Lv1 hits 2 mobs, then +1 mob per level up to 6 mobs.',weight:6,apply:()=>{if(player.class==='archer'&&(player.upgrades.piercingCandle||0)<5){player.upgrades.piercingCandle=(player.upgrades.piercingCandle||0)+1;player.pierce=player.upgrades.piercingCandle}}},{name:'Axe Reach',icon:abilityIconData.axe,rarity:'rare',desc:'Guard only: +20% axe attack range. Max level 5.',weight:6,apply:()=>{if(player.class==='guard'){player.guardRangeLevel=(player.guardRangeLevel||0)+1;player.guardRange=(player.guardRange||1.45)*1.20}}},{name:'Fireball Tome',icon:abilityIconData.fireball,rarity:'rare',desc:'Unlock or improve Fireball.',weight:5,apply:()=>player.spells.fireball++},{name:'Frost Nova Charm',icon:abilityIconData.frost,rarity:'rare',desc:'Unlock or improve Frost Nova.',weight:5,apply:()=>player.spells.frost++},{name:'Storm Acorn',icon:abilityIconData.lightning,rarity:'epic',desc:'Kills random non-boss mobs. Lv1: 3 mobs, Lv5: 7 mobs. Max level 5.',weight:4,apply:()=>{if(player.spells.lightning<5)player.spells.lightning++}},{name:'Dancing Dagger',icon:abilityIconData.dagger,rarity:'rare',desc:'Passive orbiting dagger. Each level adds one knife, max 5.',weight:5,apply:()=>{if(player.spells.dagger<5){player.spells.dagger++;syncDaggers()}}},{name:'Spirit Wisps',icon:abilityIconData.wisps,rarity:'legendary',desc:'Passive auto-shooting wisps. Rare.',weight:1,apply:()=>player.spells.wisps++},{name:'Poison Spores',icon:abilityIconData.spores,rarity:'rare',desc:'Passive toxic clouds. Upgrades increase damage.',weight:5,apply:()=>player.spells.spores++},{name:'Rune Mines',icon:abilityIconData.mines,rarity:'epic',desc:'Passive explosive mines.',weight:4,apply:()=>player.spells.mines++},{name:'Force Field',icon:abilityIconData.force,rarity:'epic',desc:'Passive aura that damages every enemy around you.',weight:3,apply:()=>player.spells.force++},{name:'Meteor Rain',icon:abilityIconData.meteor,rarity:'legendary',desc:'Clears only normal mobs. Bosses are ignored. Cooldown 90 seconds.',weight:1,apply:()=>{player.spells.meteor=1}},{name:'Blue Tea',icon:abilityIconData.tea,rarity:'basic',desc:'+25 max mana and regen.',weight:6,apply:()=>{player.maxMana+=25;player.manaRegen+=4;player.mana=player.maxMana;player.upgrades.blueTea=(player.upgrades.blueTea||0)+1}}];
...
function sanitizePlayerVitals(){
  if(!Number.isFinite(player.maxHp)||player.maxHp<=0)player.maxHp=({mage:100,guard:125,archer:85}[player.class]||100);
...
function bossMinionSpellAttack(e,dt){
  ...
  if(d<6.4&&Math.random()<.35){
    ...
    if(d<r+player.r&&player.invuln<=0){damagePlayer(e.dmg*.45,.55,0x8b5cf6,10);}
    sfx('cursed');
  }else{
    makeEnemyFireball(e.x+dx/d*.65,e.z+dz/d*.65,dx/d*(e.fireSpeed||3.9),dz/d*(e.fireSpeed||3.9),e.fireDmg||e.dmg*.45);
    sfx('cursed');
  }
}
function bossShockwave(e){
  const r=e.finalBoss?6.2:4.5, dmg=e.dmg*(e.finalBoss?.68:.42);
...
function bossCursedBolts(e,count=6){
  for(let i=0;i<count;i++){const a=(Math.PI*2/count)*i+(Math.random()-.5)*.22;makeEnemyFireball(e.x+Math.cos(a)*.9,e.z+Math.sin(a)*.9,Math.cos(a)*(e.fireSpeed||3.6),Math.sin(a)*(e.fireSpeed||3.6),(e.fireDmg||e.dmg*.32));}
...
function bossFireVolley(e,count=3){
  ...
  for(let i=0;i<count;i++){const off=(i-(count-1)/2)*.20,a=base+off;makeEnemyFireball(e.x+Math.cos(a)*1.15,e.z+Math.sin(a)*1.15,Math.cos(a)*speed,Math.sin(a)*speed,(e.fireDmg||e.dmg*.35));}
  sfx('fireball');notice((e.name||'Boss')+' launches fireballs!');
}
function bossCharge(e){
  const dx=player.x-e.x,dz=player.z-e.z,d=Math.hypot(dx,dz)||1;e.dashVx=dx/d*(e.finalBoss?13.5:10.2);e.dashVz=dz/d*(e.finalBoss?13.5:10.2);e.dashTime=e.finalBoss?.42:.34;e.dashDamage=e.dmg*(e.finalBoss?1.05:.58);burst(e.x,e.z,e.finalBoss?0xff5a2f:0xf97316,18);notice((e.name||'Boss')+' dashes toward you!');
}
function bossSpecialAttack(e,dt){
  ...
  else if(special==='finalDashVolley'){ if(Math.random()<.52)bossCharge(e); else bossFireVolley(e,5); }
  else bossCharge(e);
}
...
function bossSkillsForSlot(slot,finalFight=false){
  const base=['charge','fireVolley','summon','shockwave'];
  if(finalFight||slot>=5)return base;
...
function spawnBoss(finalFight=false){if(state.portal)return;sfx('boss');state.bossCount++;
  ...
  const bossDmgPower=finalFight?1.48:Math.pow(1.10,ordinal-1);
  const dmgProgress=1+state.mapIndex*.025;
  const speedProgress=Math.pow(1.05,ordinal-1);
  ...
  const e={tex:profile.tex,name:profile.name,x:finalFight?4:player.x+Math.cos(a)*d,z:finalFight?0:player.z+Math.sin(a)*d,
    ...
    dmg:profile.baseDmg*.92*bossDmgPower*dmgProgress,
    ...
    fireSpeed:(profile.fireSpeed||4.6)*(finalFight?1.10:1),fireDmg:(profile.fireDmg||42)*bossDmgPower*(1+state.wave*.014),
...
function saveRefreshProgress(){
  try{
    const keepState=['time','wave','waveTime','lastWave','spawnTimer','kills','bossSpawnedWave','bossCount','mapIndex','portalSpawned','finalBossSpawned','bossesDefeatedThisMap','totalXpGained','heartSpawnTimer','lastShot','currentProfile','waveWaitingForBoss','waveSpawnRemaining','waveAdvanceLock','deathMapIndex'];
    ...
    normalizeMerchantMeta();const snap={state:{},player:{},meta:clonePlain(meta),entities:snapshotEntities()};
...
function restoreRefreshProgress(){
  ...
  try{
    ...
    Object.assign(state,{running:true,paused:false,enemies:[],bullets:[],drops:[],particles:[],spells:[],enemyShots:[],orbiters:[],portal:null,lastShot:-9,paidRun:false,uiTimer:0},snap.state||{});if(profileName)state.currentProfile=profileName;
    normalizeRestoredPlayer(snap.player||{});
    if(snap.meta){resetMerchantMeta();Object.assign(meta,snap.meta);normalizeMerchantMeta();saveMerchantMeta();}
    makeDungeon();
...
function castFrostNova(){const l=player.spells.frost;if(!l)return toast('Find Frost Nova Charm first.');if(!spend(28,'frost',getCooldownMax('frost')))return;const r=3.4+l*.55,w=circle(r,0xa8e7ff,.32);w.position.set(player.x,.1,player.z);entityGroup.add(w);sfx('frost');state.spells.push({mesh:w,life:.45,scale:0});for(const e of state.enemies)if(Math.hypot(e.x-player.x,e.z-player.z)<r){e.hp-=18+l*10;e.freeze=1.6+l*.25}notice('Used Frost Nova')}
...
function castMeteorRain(){const l=player.spells.meteor;if(!l)return toast('Find Meteor Rain first.');if(!spend(80,'meteor',getCooldownMax('meteor')))return;sfx('meteor');let killed=0;const targets=state.enemies.filter(e=>!e.boss);for(const e of targets){for(let i=0;i<2;i++){const m=circle(.45,0xff733f,.75);m.position.set(e.x+(Math.random()-.5)*1.2,.2,e.z+(Math.random()-.5)*1.2);entityGroup.add(m);state.spells.push({mesh:m,life:.55,scale:.25})}e.hp=0;killed++;burst(e.x,e.z,0xff733f,18)}cleanupDead();toast('Meteor Rain!');notice('Meteor Rain cleared '+killed+' normal mobs')}
...
function weightedChoices(pool,count){const result=[],copy=pool.filter(a=>!(a.name==='Dancing Dagger'&&player.spells.dagger>=5)&&!(a.name==='Fleet Boots'&&player.speedLevel>=5)&&!(a.name==='Faster Draw'&&player.attackSpeedLevel>=5)&&!(a.name==='Sharper Runes'&&(player.upgrades.sharperRunes||0)>=5)&&!(a.name==='Meteor Rain'&&player.spells.meteor>=1)&&!(a.name==='Storm Acorn'&&player.spells.lightning>=5)&&!(a.name==='Twin Arrows'&&player.class==='guard')&&!(a.name==='Twin Arrows'&&(player.upgrades.twinArrows||0)>=5)&&!(a.name==='Piercing Candle'&&player.class!=='archer')&&!(a.name==='Piercing Candle'&&(player.upgrades.piercingCandle||0)>=5)&&!(a.name==='Axe Reach'&&player.class!=='guard')&&!(a.name==='Axe Reach'&&(player.guardRangeLevel||0)>=5));while(result.length<count&&copy.length){const total=copy.reduce((s,a)=>s+(a.weight||1),0);let r=Math.random()*total,idx=0;for(;idx<copy.length;idx++){r-=copy[idx].weight||1;if(r<=0)break}result.push(copy.splice(Math.min(idx,copy.length-1),1)[0])}return result}
function openLevelUp(){clearControls();state.paused=true;ui.levelModal.classList.remove('hidden');ui.cards.innerHTML='';weightedChoices(abilities,3).forEach(a=>{const el=document.createElement('button');el.className='card rarity-'+(a.rarity||'basic');el.innerHTML=`<b><span class="pick-icon">${iconHtml(a.icon)}</span>${a.name}<em class="rarity-tag">${rarityLabel(a.rarity||'basic')}</em></b><span>${a.desc}</span>`;el.onclick=()=>{const before=JSON.stringify(player.spells);a.apply();ui.levelModal.classList.add('hidden');clearControls();state.paused=false;toast(a.name);notice((before!==JSON.stringify(player.spells)?'Unlocked / upgraded: ':'Upgrade chosen: ')+a.name)};ui.cards.appendChild(el)})}
function toast(m){return}
function notice(m){const allowed=/^(Wave \d+ started|Mini boss appeared!|Main boss appeared!|Boss appeared!|Last boss appeared!|Portal opened!|Purple crystal absorbed)/;if(!allowed.test(String(m)))return;const el=document.createElement('div');el.className='event-msg';el.textContent=m;ui.eventFeed.appendChild(el);setTimeout(()=>el.remove(),3100);while(ui.eventFeed.children.length>4)ui.eventFeed.firstChild.remove()}
...
function makeSaveSnapshot(){
  const keepState=['time','wave','waveTime','lastWave','spawnTimer','kills','bossSpawnedWave','bossCount','mapIndex','portalSpawned','finalBossSpawned','bossesDefeatedThisMap','totalXpGained','heartSpawnTimer','lastShot','currentProfile','waveWaitingForBoss','waveSpawnRemaining','waveAdvanceLock','deathMapIndex'];
  const keepPlayer=['class','x','z','hp','maxHp','mana','maxMana','speed','level','xp','nextXp','money','damage','fireRate','attackSpeedLevel','bullets','pierce','bulletSpeed','magnet','manaRegen','crit','spread','speedLevel','guardRange','guardRangeLevel','upgrades','spells','cooldowns'];
  normalizeMerchantMeta();const snap={version:3,savedAt:Date.now(),state:{},player:{},meta:clonePlain(meta),entities:snapshotEntities()};
  keepState.forEach(k=>snap.state[k]=state[k]);
  keepPlayer.forEach(k=>snap.player[k]=JSON.parse(JSON.stringify(player[k])));
  return snap;
}
...
function saveGameToProfile(){
  ...
  normalizeMerchantMeta();saveMerchantMeta();
  const saves=readProfileSaves();
  saves[state.currentProfile]=makeSaveSnapshot();
  saveMerchantMeta();
  const ok=writeProfileSaves(saves);
  if(ok)saveMerchantMeta();
  if(ui.pauseSaveStatus)ui.pauseSaveStatus.textContent=ok?'Saved: '+state.currentProfile:'Save failed — browser storage may be full.';
  return ok;
}
function restoreGameSnapshot(snap,profileName){
  if(!snap||!snap.state||!snap.player)return false;
  document.body.classList.remove('title-flow');
  clearRuntimeVisuals();
  Object.assign(state,{running:true,paused:false,enemies:[],bullets:[],drops:[],particles:[],spells:[],enemyShots:[],orbiters:[],portal:null,lastShot:-9,paidRun:false,uiTimer:0},snap.state||{});if(profileName)state.currentProfile=profileName;
  normalizeRestoredPlayer(snap.player||{});
  if(!Number.isFinite(state.waveSpawnRemaining))state.waveSpawnRemaining=waveMobTarget();
  if(snap.meta){resetMerchantMeta();Object.assign(meta,snap.meta);normalizeMerchantMeta();saveMerchantMeta();}
  makeDungeon();
  finishRestorePlayerPosition();
  restoreEntities(snap.entities);
  ...
  state.paused=false;
  updateUi();
  notice('Wave '+state.wave+' started — save loaded.');
  return true;
}
function renderSaveList(){
  ...
  ui.saveList.querySelectorAll('[data-load-profile]').forEach(btn=>btn.addEventListener('click',()=>{const all=readProfileSaves();restoreGameSnapshot(all[btn.dataset.loadProfile],btn.dataset.loadProfile);}));
...
function applyClassStats(cls){player.class=cls||player.class||'mage';player.mesh.scale.setScalar(1.45);if(player.class==='mage'){player.mesh.scale.setScalar(1.85);player.mesh.material.map=textures.hero;player.fireRate=.48;player.bullets=Math.max(1,player.bullets)}else if(player.class==='guard'){player.mesh.material.map=textures.guardHero;player.fireRate=.58;player.bullets=1}else if(player.class==='archer'){player.mesh.material.map=textures.archerHero;player.fireRate=.34;player.bullets=Math.max(1,player.bullets);player.bulletSpeed*=1.2}player.mesh.material.needsUpdate=true}
function reset(cls,startMapIndex=0){document.body.classList.remove('title-flow');if(cls)player.class=cls;[...state.enemies,...state.bullets,...(state.enemyShots||[]),...state.drops,...state.particles,...state.spells,...state.orbiters].forEach(removeEntityVisuals);Object.assign(state,{running:true,paused:false,time:0,wave:1,waveTime:0,lastWave:1,spawnTimer:0,enemies:[],bullets:[],drops:[],particles:[],spells:[],enemyShots:[],orbiters:[],portal:null,lastShot:-9,kills:0,paidRun:false,bossSpawnedWave:0,bossCount:0,mapIndex:Math.max(0,Math.min(mapDefs.length-1,Number(startMapIndex)||0)),portalSpawned:false,finalBossSpawned:false,bossesDefeatedThisMap:0,totalXpGained:0,heartSpawnTimer:15,uiTimer:0,waveSpawnRemaining:0,waveAdvanceLock:0});const base=HERO_BASE_STATS[player.class]||HERO_BASE_STATS.mage;const startHp=base.hp,startMana=100;Object.assign(player,{x:-4,z:4,hp:startHp,maxHp:startHp,mana:startMana,maxMana:startMana,speed:6,level:1,xp:0,nextXp:24,money:0,damage:base.dmg,fireRate:.48,attackSpeedLevel:0,bullets:1,pierce:0,bulletSpeed:13,magnet:2.2,manaRegen:11,crit:.05,spread:.13,invuln:0,speedLevel:0,guardRange:1.45,guardRangeLevel:0,upgrades:{},spells:{fireball:0,frost:0,lightning:0,dagger:0,wisps:0,spores:0,mines:0,force:0,meteor:0},cooldowns:{fireball:0,frost:0,lightning:0,meteor:0,wisp:0,spore:0,mine:0,force:0}});applyMerchantRunBonuses();applyClassStats(player.class);makeDungeon();state.waveSpawnRemaining=waveMobTarget();notice('Wave 1 started — clear all enemies to start the next wave. Boss every 2 waves in asset order.');ui.welcomeModal.classList.add('hidden');ui.instructionModal.classList.add('hidden');ui.startModal.classList.add('hidden');ui.gameOverModal.classList.add('hidden');ui.victoryModal.classList.add('hidden');ui.levelModal.classList.add('hidden')}
function grantStarterSkills(){/* deprecated: merchant wheel spells are now stored in meta.wheelSpells and applied by applyMerchantRunBonuses() */}
function showMerchantMessage(msg){
  if(ui.finalStats){
    const old=ui.finalStats.innerHTML||'';
    const base=old.replace(/<br><b class="merchant-msg">.*?<\/b>/,'');
    ui.finalStats.innerHTML=base+'<br><b class="merchant-msg">'+String(msg).replace(/[<>&]/g,c=>({'<':'&lt;','>':'&gt;','&':'&amp;'}[c]))+'</b>';
  }
  notice(String(msg));
}
function renderShop(){
  ...
  ui.shop.innerHTML='';
  if(ui.finalStats){
    const diedMap=Number.isFinite(state.deathMapIndex)?state.deathMapIndex:state.mapIndex;
    ui.finalStats.innerHTML=`Level ${player.level}, wave ${state.wave}, map ${diedMap+1}, ${state.kills} monsters defeated, $${player.money} collected.<br>Merchant coins: $${meta.gold}`;
  }
  for(const item of shopItems){
    const level=meta[item.key]||0,max=item.max??99,cost=item.costFor?item.costFor(level):(item.base+level*item.base);
    const btn=document.createElement('button');
    btn.disabled=level>=max;
    btn.innerHTML=`<b>${item.name}</b><small>${item.desc}</small><small>Level ${level}${item.max?'/'+item.max:''} · Cost: $${level>=max?'MAX':cost}</small>`;
    btn.onclick=()=>{if(level>=max)return;if(meta.gold>=cost){meta.gold-=cost;item.apply();persistMerchantAndProfile();renderShop();updateUi();showMerchantMessage('Bought '+item.name)}else showMerchantMessage('Not enough coins')};
    ui.shop.appendChild(btn);
  }
  const wheel=document.createElement('button');
  const wheelMax=3,wheelCost=500,wheelLevel=meta.wheelSpins||0;
  wheel.disabled=wheelLevel>=wheelMax;
  const owned=(meta.wheelSpells||[]).map(id=>merchantSpellNames[id]||id).join(', ')||'none yet';
  wheel.innerHTML=`<b>Wheel of Fortune</b><small>Spin for 1 random active or passive spell. Max 3 purchases.</small><small>Spins ${wheelLevel}/${wheelMax} · Cost: ${wheelLevel>=wheelMax?'MAX':'$'+wheelCost}</small><small>Won: ${owned}</small>`;
  wheel.onclick=()=>{
    normalizeMerchantMeta();
    if(meta.wheelSpins>=wheelMax)return;
    if(meta.gold<wheelCost){showMerchantMessage('Not enough coins for Wheel of Fortune');return;}
    meta.gold-=wheelCost;meta.wheelSpins++;
    wheel.disabled=true;
    wheel.innerHTML='<b><span class="wheel-spin">✦</span>Wheel of Fortune spinning...</b><small>Drawing from all active and passive abilities...</small>';
    showMerchantMessage('Wheel of Fortune is spinning...');
    const spinTime=1200;
    let flashes=0;
    const spinNames=merchantWheelPool.map(id=>merchantSpellNames[id]||id);
    const spinAnim=setInterval(()=>{
      flashes++;
      const label=spinNames[Math.floor(Math.random()*spinNames.length)];
      wheel.innerHTML='<b><span class="wheel-spin">✦</span>Wheel of Fortune</b><small>Rolling: '+label+'</small>';
      if(flashes>8)clearInterval(spinAnim);
    },120);
    setTimeout(()=>{
      clearInterval(spinAnim);
      const id=merchantWheelPool[Math.floor(Math.random()*merchantWheelPool.length)];
      meta.wheelSpells.push(id);
      if(state.running)grantMerchantSpell(id,true);
      persistMerchantAndProfile();
      renderShop();updateUi();showMerchantMessage('Wheel won: '+(merchantSpellNames[id]||id));
    },spinTime);
  };
  ui.shop.appendChild(wheel);
}
function startClass(cls,startMapIndex=0){clearControls();reset(cls||'mage',startMapIndex)}
if(ui.newGameBtn)ui.newGameBtn.addEventListener('click',()=>{audioStart();sfx('ui');openNewGameProfile()});if(ui.loadGameBtn)ui.loadGameBtn.addEventListener('click',()=>{audioStart();sfx('ui');openLoadGame()});if(ui.profileCreateBtn)ui.profileCreateBtn.addEventListener('click',()=>{audioStart();sfx('ui');createProfileAndChooseHero()});if(ui.profileBackBtn)ui.profileBackBtn.addEventListener('click',()=>{ui.profileModal.classList.add('hidden');ui.welcomeModal.classList.remove('hidden')});if(ui.loadBackBtn)ui.loadBackBtn.addEventListener('click',()=>{ui.loadModal.classList.add('hidden');ui.welcomeModal.classList.remove('hidden')});if(ui.welcomeNextBtn)ui.welcomeNextBtn.addEventListener('click',()=>{audioStart();sfx('ui');ui.welcomeModal.classList.add('hidden');if(ui.storyModal)ui.storyModal.classList.remove('hidden');else ui.instructionModal.classList.remove('hidden')});if(ui.storyNextBtn)ui.storyNextBtn.addEventListener('click',()=>{audioStart();sfx('ui');ui.storyModal.classList.add('hidden');ui.instructionModal.classList.remove('hidden')});ui.instructionNextBtn.addEventListener('click',()=>{audioStart();sfx('ui');ui.instructionModal.classList.add('hidden');ui.startModal.classList.remove('hidden')});ui.mageBtn.addEventListener('click',()=>{audioStart();sfx('ui');startClass('mage')});ui.guardBtn.addEventListener('click',()=>{audioStart();sfx('ui');startClass('guard')});ui.archerBtn.addEventListener('click',()=>{audioStart();sfx('ui');startClass('archer')});ui.restartBtn.addEventListener('click',()=>{audioStart();sfx('ui');const map=Number.isFinite(state.deathMapIndex)?state.deathMapIndex:(Number.isFinite(state.lastRunMapIndex)?state.lastRunMapIndex:state.mapIndex);startClass(player.class||'mage',map)});ui.victoryRestartBtn.addEventListener('click',()=>{audioStart();sfx('ui');startClass(player.class||'mage',0)});
...
function waveMobTarget(){const wave=Math.max(1,state.wave||1),map=Math.max(0,state.mapIndex||0);return Math.max(5,Math.round(5+wave*2+map*4));}
...
function balancedSpawnInterval(){const wave=Math.max(1,state.wave||1),map=Math.max(0,state.mapIndex||0);const base=1.15-(wave-1)*.045-map*.045;const bossPressure=state.enemies.some(e=>e.boss)?-.04:0;return Math.max(.38,base+bossPressure)}
function balancedSpawnCount(){const wave=Math.max(1,state.wave||1),map=Math.max(0,state.mapIndex||0);const waveRamp=Math.floor((wave-1)/3);const mapRamp=Math.floor(map/2);const bossBonus=state.enemies.some(e=>e.boss)?1:0;const randomBonus=Math.random()<Math.min(.45,.08*wave+.05*map)?1:0;return Math.min(5,1+waveRamp+mapRamp+bossBonus+randomBonus)}
...
function renderSideStats(){
  ...
  if(u.twinArrows)push(abilityIconData.twin,'Twin Arrows','Lv '+u.twinArrows,abilityRarity['Twin Arrows']);
  ...
  if(u.sharperRunes)push(abilityIconData.sharper,'Sharper Runes','Lv '+u.sharperRunes,abilityRarity['Sharper Runes']);
  if(player.speedLevel)push(abilityIconData.boots,'Fleet Boots','Lv '+player.speedLevel+'/5',abilityRarity['Fleet Boots']);
  if(u.moonMagnet)push(abilityIconData.magnet,'Moon Magnet','Lv '+u.moonMagnet,abilityRarity['Moon Magnet']);
  if(u.warmSoup)push(abilityIconData.heart,'Warm Soup','Lv '+u.warmSoup,abilityRarity['Warm Soup']);
  ...
  if(u.blueTea)push(abilityIconData.tea,'Blue Tea','Lv '+u.blueTea,abilityRarity['Blue Tea']);
  ...
  for(const [id,[icon,name,abilityName]] of Object.entries(spellNames)){
    ...
    if(lvl)push(icon,name,id==='dagger'?`Lv ${lvl}/5`:(id==='meteor'?'Unlocked':'Lv '+lvl),abilityRarity[abilityName]||'basic');
  }
  ui.sideAbilityList.innerHTML=items.length?items.join(''):'<div class="side-empty">No abilities yet</div>';
}
function renderAbilityBar(){ui.abilityBar.innerHTML='';for(const def of abilityDefs){const lvl=player.spells[def.id]||0;if(!lvl)continue;const el=document.createElement('div');el.className='ability-icon '+def.type;el.title=def.name;const cd=def.cd?player.cooldowns[def.cd]:0;if(def.type==='active'&&cd<=0)el.classList.add('ready');el.innerHTML=`<span>${iconHtml(def.icon)}</span>${def.key?`<span class="ability-key">${def.key}</span>`:''}<span class="ability-level">${def.id==='dagger'?lvl+'/5':'Lv '+lvl}</span>`;if(def.type==='active'&&cd>0){const max=getCooldownMax(def.id),pct=Math.max(0,Math.min(100,cd/max*100)),fill=document.createElement('div'),timer=document.createElement('div');fill.className='ability-fill';fill.style.height=pct+'%';timer.className='ability-cd';timer.textContent=cd>=10?Math.ceil(cd):cd.toFixed(1);el.appendChild(fill);el.appendChild(timer)}ui.abilityBar.appendChild(el)}}
function drawMinimap(){if(!ui.minimap)return;const ctx=ui.minimap.getContext('2d'),w=ui.minimap.width,h=ui.minimap.height,b=mapBounds;ctx.clearRect(0,0,w,h);ctx.fillStyle='rgba(9,6,13,.92)';ctx.fillRect(0,0,w,h);const sx=w/(b.maxX-b.minX+1),sz=h/(b.maxZ-b.minZ+1);ctx.fillStyle='rgba(117,104,82,.42)';for(const cell of floorSet){const parts=cell.split(','),x=Number(parts[0]),z=Number(parts[1]);ctx.fillRect((x-b.minX)*sx,(z-b.minZ)*sz,Math.max(1,sx),Math.max(1,sz))}ctx.strokeStyle='rgba(255,207,123,.22)';ctx.strokeRect(.5,.5,w-1,h-1);if(state.portal){const px=(state.portal.x-b.minX)*sx,pz=(state.portal.z-b.minZ)*sz;ctx.fillStyle='#d8b4fe';ctx.beginPath();ctx.arc(px,pz,5,0,Math.PI*2);ctx.fill();ctx.strokeStyle='#fff';ctx.lineWidth=1;ctx.stroke()}else{const ps=portalSpot(),px=(ps.x-b.minX)*sx,pz=(ps.z-b.minZ)*sz;ctx.strokeStyle='rgba(216,180,254,.55)';ctx.lineWidth=2;ctx.setLineDash([3,3]);ctx.beginPath();ctx.arc(px,pz,5,0,Math.PI*2);ctx.stroke();ctx.setLineDash([])}for(const d of state.drops){const dx=(d.x-b.minX)*sx,dz=(d.z-b.minZ)*sz;if(d.kind==='heart'){ctx.fillStyle='#ff77b7';ctx.beginPath();ctx.moveTo(dx,dz+3);ctx.bezierCurveTo(dx-6,dz-1,dx-4,dz-7,dx,dz-3);ctx.bezierCurveTo(dx+4,dz-7,dx+6,dz-1,dx,dz+3);ctx.fill();ctx.strokeStyle='rgba(255,255,255,.75)';ctx.lineWidth=.8;ctx.stroke();}else if(d.kind==='levelCrystal'){ctx.save();ctx.translate(dx,dz);ctx.rotate(Math.PI/4);ctx.fillStyle='#a855f7';ctx.fillRect(-4,-4,8,8);ctx.strokeStyle='#f5d0fe';ctx.lineWidth=1.2;ctx.strokeRect(-4,-4,8,8);ctx.restore();ctx.fillStyle='#ffffff';ctx.globalAlpha=.65;ctx.beginPath();ctx.arc(dx-1.5,dz-1.5,1.2,0,Math.PI*2);ctx.fill();ctx.globalAlpha=1;}}ctx.fillStyle='#ff5a5f';for(const e of state.enemies){const ex=(e.x-b.minX)*sx,ez=(e.z-b.minZ)*sz;ctx.beginPath();ctx.arc(ex,ez,e.boss?3.5:2,0,Math.PI*2);ctx.fill()}ctx.fillStyle='#9ee7ff';ctx.beginPath();ctx.arc((player.x-b.minX)*sx,(player.z-b.minZ)*sz,3.5,0,Math.PI*2);ctx.fill()}
